...
class OptionBase(ABC):
    def __init__(self, env, subgoal_index: int, gamma: float):
        self.env = env
    ...
    def get_arrow_data(self):
        """
        For each state in this option, pick the greedy action under self.Q,
        then return the tuple (x_pos, y_pos, x_dir, y_dir, color, coords_list)
        expected by GridEnvWrapper.get_arrow_data.
        """
        actions = []
        qvals = []
        states = []
        ...
        for state_idx in range(self.n_states):
            coords = self.env.state_to_coords[state_idx]
    ...
    def plot_q_vals(self, activation_data=None, save_path: Optional[str] = None, show: bool = True):
        """
        Plot this option’s greedy Q‐policy as arrows over the grid.

        Args:
            activation_data: extra overlay data (same as your SF version)
            save_path:       full filepath to save the figure (or None to skip saving)
            show:            whether to display the plot interactively
        """
        # 1) Get the arrow‐vector data for this option
        arrow_data = self.get_arrow_data()

        # 2) Call existing plotting utility.
        plot_q_vals(
            env=self.env,
...
class OptionVI(OptionBase):
    def __init__(self, env, subgoal_index: int, gamma: Optional[float] = 1):
            ...
            for state_idx in range(self.n_states):
                ...
                for aidx in range(self.n_actions):
                    ...
                    for next_state_idx in range(self.n_states):
                        ...
                        q_value += prob * (
                                self.env.reward(self.env.state_to_coords[state_idx]) + (1 - done) * self.gamma *
                                V[next_state_idx]
                        )
